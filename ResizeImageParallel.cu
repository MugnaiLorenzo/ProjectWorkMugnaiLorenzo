...
__global__ void convolveKernel_NonVec(unsigned char* input, unsigned char* output,
                                      float* kernel, int width, int height, int ksize) {
    ...
    for (int i = -offset; i <= offset; ++i) {
        for (int j = -offset; j <= offset; ++j) {
            ...
            if (nx >= 0 && nx < width && ny >= 0 && ny < height) {
                ...
                acc += input[idx] * kernel[(i + offset) * ksize + (j + offset)];
            }
        }
    }
...
unsigned char* applyConvolutionCUDA(const unsigned char* input, int width, int height,
    ...
    cudaMalloc(&d_kernel, sizeof(float) * klen);

    cudaMemcpy(d_input, input, imgSize, cudaMemcpyHostToDevice);
    cudaMemcpy(d_kernel, kernel1D.data(), sizeof(float) * klen, cudaMemcpyHostToDevice);
    ...
    if (type == NonVectorized) {
        ...
        convolveKernel_NonVec<<<grid, block3D>>>(d_input, d_output, d_kernel, width, height, ksize);
    } else {
        convolveKernel_Vec<<<grid, block>>>(d_input, d_output, d_kernel, width, height, ksize);
    }
